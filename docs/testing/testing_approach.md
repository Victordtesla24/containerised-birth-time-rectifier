...
## ERROR RESOLUTION:
  ...
  5. REMOVE MOCKEDUP/DUPLICATE OR SIMULATED CODE FORM PYTHON FILE EVERYTIME YOU IMPLEMENT OR REPLACE CODE BY RUNNING THE FOLLOWING TERMINAL COMMAND
     ```bash
     rg --pcre2 --type-add 'py:*.py' --type py \
     -C 2 --vimgrep --smart-case \
     -e '(?i)\b(mockup|fallback|in\s+real\s+impl(ementation)?|this\s+is\s+a\s+fallback|later\s+impl(ement)?|for\s+testing|stubs?|simulated)\b' \
     -e '@mock|MagicMock|@patch' \
     -e 'TODO|FIXME|HACK|XXX' \
     -e '\b(True|False|null)\b(?![ \t]*#)' \
     -e '"[^"]{30,}"|\b\d{5,}\b|\b[A-Za-z0-9]{32}\b' \
     --glob='!tests/*' --glob='!venv/*' ai_service/ api_gateway/
     ```
...
## TASKS:
1. YOU ARE REQUIRED TO SYSTEMATICALLY REVIEW, ANALYSE AND INSPECT DIRECTORY STRUCTURE (ATTACHED) & PYTHON PRODUCTION FILES ONE BY ONE TO FIND ANY AND ALL DUPLICATION, MOCKED UP CODE, SIMULATED FALLBACK MECHANISMS, HARDCODED VALUES, ERROR MASKING OR WARNING SUPRESSIONS IN THE PRODUCTION PYTHON FILES LOCATED WITHIN @ai_service & @api_gateway DIRECTORIES.

2. YOU THEN NEED TO DOCUMENT THE DUPLICATION IN THE @duplicate_code_identifier.md DOCUMENT.

3. AND THEN AFTER DOCUMENTATION, YOU MUST SYSTEMATICALLY REMOVE EVERY SINGLE DUPLICATE FUNCTION, LINE OF CODE, FILE AND SCRIPT YOU IDENTIFIED ONE BY ONE FROM EVERY PYTHON FILE YOU HAVE INSPECTED, EXAMINED AND REVIEWED IN THE PRODUCTION DIRECTORIES, BUT "WITHOUT BREAKING THE CODE OR THE APPLICATION FUNCTIONALITY" KEEPING THE CODE AND APPLICATION FUNCTIONALITY INTACT.

4. YOU MUST ALSO VERIFY, AND IMPLEMENT THE PRODUCTION FILES INLINE WITH THE FUNCTIONAL REQUIREMENTS FOR THE APPLICATION FLOW DETAILED IN THE "ORIGINAL SEQUENCE DIAGRAM - THE FULL IMPLEMENTATION" SECTION OF THE @sequence_diagram.md, WHICH ADHERES TO THE API & WEBSOCKET ARCHITECTURE DETAILED ON @api_architecture.md @unified_api_gateway_diagram.mdSATISFYING TESTING REQUIREMENTS DETAILED IN @testing_approach.md DOCUMENT. THIS MEANS WHEN A MOCKEDUP/DUPLICATE OR SIMULATED CODE IS REMOVED - YOU ARE REQUIRED TO REPLACE IT WITH THE REAL, ACTUAL AND PRODUCTION LEVEL IMPLEMENTATION WITH REAL API CALLS, API ENDPOINTS, API INPUT AND RESPOSE STRUCTURES, REAL API KEYS (@.env ), ACTUAL & ACCURATE ASTROLOGICAL CALCULATIONS, BIRTH CHART & PLANETARY POSITIONS, REALTIME GEOCODES AND OPENAI CHART VERIFICATIONS USING OPENAI API KEY (@.env ), REAL TIME DYNAMIC, AND PERSONALISED QUESTIONNAIRE AND BIRTH TIME RECTIFICATION ALGORITHM IMPLEMENTATION WITH ACCORDANCE TO THE DOCUMENTATION PROVIDED.

5. DO NOT CONSIDER YOUR TASK TO BE COMPLETED UNTIL ALL 180+ PRODUCTION PYTHON FILES ARE EXAMINED, INSPECTED, REVIEWED AND DUPLICATE/MOCKEDUP/SIMULATED FALLBACK MECHANISM IN THE CODE/HARD CODED VALUES IN FUNCTIONS, LINES OF CODE, FILES, CODE SNNIPPETS OR SCRIPTS ARE PRECISELY AND ACCURATELY REMOVED.
