...
# Birth Time Rectifier - Test Runner Common Functions
# Extracted from run_all_tests.sh for reuse in integrated testing scripts
...
run_test() {
  ...
  # Check for skipped tests before executing
  local skipped=false
  if $TREAT_WARNINGS_AS_ERRORS; then
    # Add warning flags to pytest and jest
    if [[ "$command" == *"pytest"* ]]; then
      command="$command -W error"
    elif [[ "$command" == *"npm test"* ]]; then
      command="$command --no-warnings=false"
    fi
  fi
  ...
  if grep -q -E "(SKIPPED|skip|xdescribe|xit|test\.skip)" "$log_file"; then
    if ! $ALLOW_SKIPS; then
      echo -e "${RED}⚠️ Test contains skipped tests!${NC}"
      echo -e "${YELLOW}Tests that are skipped must be fixed or properly removed.${NC}"
      echo -e "${YELLOW}If you need to temporarily allow skips, run with --allow-skips${NC}"
      # Extract the skipped tests to show them
      echo -e "${YELLOW}Skipped tests found:${NC}"
      grep -E "(SKIPPED|skip|xdescribe|xit|test\.skip)" "$log_file" | head -5 | sed 's/^/  /'

      # Treat skipped tests as failures
      exit_code=1
      skipped=true
    else
      echo -e "${YELLOW}⚠️ Warning: Test contains skipped tests (allowed by --allow-skips)${NC}"
    fi
  fi
  ...
  if [ $exit_code -eq 0 ]; then
    ...
    if [ $skipped = true ]; then
      echo -e "${RED}❌ FAILED${NC} (Skipped tests detected) (${execution_time}s)"
    else
    ...
    # Exit if continue on error is not enabled or if we need to fail fast
    if ( ! $CONTINUE_ON_ERROR ) || ( $FAIL_FAST && ! $skipped ); then
...
run_standard_tests() {
  ...
  run_test_group "Unit Tests" \
    ...
    "Backend" "Python Tests" "pytest -xvs ai_service/tests/"
  ...
  if check_services; then
    ...
    if ! $ALLOW_SKIPS; then
      echo -e "${RED}Services are not running. Integration tests CANNOT be skipped.${NC}"
      echo -e "${RED}Please start the required services and try again.${NC}"
      echo -e "${YELLOW}If you need to temporarily bypass this check, run with --allow-skips${NC}"
      set_test_result "Integration::React Integration Tests" "fail"
      exit 1
    else
      echo -e "${YELLOW}Warning: Services are not running. Skipping integration tests (allowed by --allow-skips)${NC}"
      echo -e "${YELLOW}Note: Skipping tests may hide serious issues and is not recommended for CI/CD environments${NC}"
