...
print_help() {
    ...
    echo "  -c, --compose       Use docker-compose instead of direct docker run"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 -c -s                      # Use docker-compose in server mode"
    echo ""
}
...
# Run using docker-compose or direct docker
if [ "$USE_COMPOSE" = true ]; then
    echo "🚀 Running analysis using docker-compose..."

    # Export variables for docker-compose
    export ANALYZER_ARGS
    export SERVER_MODE

    # Run docker-compose
    docker-compose -f docker-compose.duplication.yml up --build

    # Clean up when done (if not in server mode)
    if [ "$SERVER_MODE" = false ]; then
        docker-compose -f docker-compose.duplication.yml down
    fi
else
    # Detect GPU
