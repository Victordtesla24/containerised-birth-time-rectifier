...
# Advanced health check with multiple fallbacks and improved reliability
HEALTHCHECK --interval=30s --timeout=30s --start-period=30s --retries=5 \
    CMD curl -s -f http://localhost:8000/api/v1/ai/status || curl -s -f http://localhost:8000/health || nc -z localhost 8000 || exit 1
...
# - increased worker count for better performance under load
