...
@router.get("/{chart_id}", response_model=ChartData)
async def get_chart(
    chart_id: str,
    chart_service: ChartService = Depends(get_chart_service)
):
    ...
    """
    try:
        chart_data = chart_service.get_chart(chart_id)
        ...
        chart = ChartData(
            ...
            houses=[HouseData(**house) for house in chart_data["houses"]],
            aspects=[AspectData(**aspect) for aspect in chart_data["aspects"]]
        )
