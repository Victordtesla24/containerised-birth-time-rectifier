...
describe('QuestionnairePage', () => {
  ...
  it('handles boolean question answers correctly', async () => {
  ...
  it('handles date question answers correctly', async () => {
    // Mock successful API responses
    const mockQuestions = [
      {
        id: 'q1',
  ...
  it('displays confidence score correctly', async () => {
    ...
    // Check confidence score with a more flexible matcher
    await waitFor(() => {
      const confidenceElement = screen.queryByText(/confidence score: 65%/i);
      expect(confidenceElement).toBeInTheDocument();
    }, { timeout: 3000 });
  });

  it('handles options type questions correctly', async () => {
    // Mock successful API responses
    const mockQuestions = [
      {
        id: 'q1',
    ...
    // Wait for question to load
    await waitFor(() => {
      const questionElement = screen.queryByText(/describe your personality/i);
      expect(questionElement).toBeInTheDocument();
    }, { timeout: 3000 });

    // Check options are rendered with more flexible matchers
    await waitFor(() => {
      expect(screen.queryByText(/introverted/i)).toBeInTheDocument();
      expect(screen.queryByText(/extroverted/i)).toBeInTheDocument();
      expect(screen.queryByText(/balanced/i)).toBeInTheDocument();
    }, { timeout: 3000 });

    // Snapshot of options question
    expect(container).toMatchSnapshot('questionnaire-with-options-question');
  });
});
