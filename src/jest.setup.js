...
const customConfig = {
  ...
  failureThreshold: 0.03, // Allow 3% difference before failing
  failureThresholdType: 'percent',
};

// Add a custom matcher with default configuration
expect.extend({
...
// Store original implementations before any mocking
const originalFetch = global.fetch;
const originalXHR = global.XMLHttpRequest;

// Prepare for handling network requests in tests
beforeAll(() => {
  // Mock XMLHttpRequest for CORS issues but in a way that propagates real errors
  const MockXHR = function() {
    return {
      open: jest.fn(),
      send: jest.fn(function() {
        // Simulate a failed request to test error handling
        if (this.onerror) {
        ...
        if (this.onreadystatechange) {
          ...
          this.response = JSON.stringify({ error: 'Service unavailable' });
          this.responseText = JSON.stringify({ error: 'Service unavailable' });
      ...
      addEventListener: jest.fn((event, callback) => {
        if (event === 'load') {
          // Don't automatically call success callback
        } else if (event === 'error') {
          // Simulate error
  ...
  // Replace XMLHttpRequest to test error handling
  global.XMLHttpRequest = MockXHR;

  // Modify Fetch API to properly propagate errors
  global.fetch = jest.fn((url, options) => {
    // Always return a failed response to test error handling
    return Promise.resolve({
      json: () => Promise.resolve({ error: 'Service unavailable for testing' }),
      text: () => Promise.resolve('Service unavailable for testing'),
      ok: false,
      status: 503,
...
console.error = (...args) => {
  ...
  // Only filter out React and UI-related warnings, keep all API and service errors visible
  const ignoredErrors = [
    ...
    // IMPORTANT: We no longer ignore service-related errors
    // This ensures tests fail appropriately when services are unavailable
  ];
...
// Response polyfill for Node environments
