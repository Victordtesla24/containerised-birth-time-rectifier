...
async function runTests() {
  ...
  const initResult = await testUtils.testEndpoint(
    ...
    `${config.apiBaseUrl}/api/initialize-questionnaire`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  if (initResult.success && initResult.data && initResult.data.sessionId) {
    ...
    await testUtils.testEndpoint(
      'Next Question API', 
      `${config.apiBaseUrl}/api/next-question`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    await testUtils.testEndpoint(
      'Analysis API',
      `${config.apiBaseUrl}/api/analysis?sessionId=${sessionId}`
    );
  }
