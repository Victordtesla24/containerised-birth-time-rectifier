...
def execute_curl_command(curl_command: str) -> Tuple[str, int]:
    """
    Execute a curl command and return the response and status code.

    Args:
        curl_command: The curl command to execute

    Returns:
        Tuple containing the response text and status code
    """
    ...
    try:
        ...
        # Get the response text and status code
        response_text = process.stdout

        # Try to extract status code from curl output
        status_code = 200  # Default to 200 if we can't extract it

        # If curl returned an error, log it
        if process.returncode != 0:
            logger.warning(f"Curl command failed with return code {process.returncode}")
            logger.warning(f"Error: {process.stderr}")
            status_code = 500  # Default error code

            # Try to extract status code from stderr
            status_match = re.search(r'HTTP/\d+\.\d+\s+(\d+)', process.stderr)
            if status_match:
                status_code = int(status_match.group(1))
        else:
            # Try to extract status code from stdout
...
def parse_curl_response(response_text: str) -> Dict[str, Any]:
    """
    Parse the response from a curl command into a dictionary.

    Args:
        response_text: The response text from the curl command
    ...
    try:
        # Find the JSON part of the response (skip headers)
...
def format_response_data(response_data: Dict[str, Any]) -> str:
    """
    Format response data in a readable way, especially for astrological data.

    Args:
        response_data: Dictionary containing the response data

    Returns:
        Formatted string representation of the response data
    """
    if not isinstance(response_data, dict):
        return str(response_data)

    # Format based on content type
    if "planets" in response_data:
        ...
        if isinstance(planets, dict):
            for planet_name, planet_data in planets.items():
                if isinstance(planet_data, dict):
                    ...
                    formatted += f"{planet_name.capitalize():9} : {sign:12} {degree:.2f}° House {house}{retrograde}\n"
            ...
            for planet in planets:
                if isinstance(planet, dict):
                    ...
                    retrograde = " (R)" if planet.get("is_retrograde", False) else ""
                    formatted += f"{name:9} : {sign:12} {degree:.2f}° House {house}{retrograde}\n"

        # Add ascendant if available
        ascendant = response_data.get("ascendant", {})
        if ascendant:
        ...
        # Format chart data
        formatted = "CHART INFORMATION:\n"
        formatted += "=================\n"
        formatted += f"Chart ID: {response_data['chart_id']}\n"

        # Add verification info if available
        verification = response_data.get("verification", {})
        if verification:
            formatted += f"Verified: {verification.get('verified', False)}\n"
            formatted += f"Confidence: {verification.get('confidence', 0)}%\n"

        return formatted

    elif "differences" in response_data:
        # Format chart comparison data
        formatted = "CHART COMPARISON:\n"
        formatted += "================\n"
        ...
        if isinstance(differences, list):
            for i, diff in enumerate(differences, 1):
                if isinstance(diff, dict):
                    ...
                    formatted += f"Difference {i}: {factor}\n"
                    formatted += f"  Original: {original}\n"
                    formatted += f"  Rectified: {rectified}\n\n"

        return formatted

    elif "rectified_time" in response_data:
        # Format rectification result
        formatted = "RECTIFICATION RESULT:\n"
        formatted += "====================\n"
        formatted += f"Rectified Time: {response_data['rectified_time']}\n"

        if "confidence" in response_data:
            formatted += f"Confidence: {response_data['confidence']}%\n"
                    ...
                    if options:
                        ...
                        for j, option in enumerate(options, 1):
                            if isinstance(option, dict):
                                ...
                                formatted += f"  {j}. {option_text}\n"

        return formatted

    # Default formatting for other types of data
    return json.dumps(response_data, indent=2)

def build_curl_command(method: str, url: str, data: Optional[Dict[str, Any]] = None,
                      headers: Optional[Dict[str, str]] = None, timeout: int = 10) -> str:
    """
    Build a curl command string for the given parameters.
...
"""
    ...
    while retry_count < max_retries:
        try:
            ...
            curl_command = build_curl_command("GET", SESSION_ENDPOINT, timeout=10)

            # Execute curl command
            response_text, status_code = execute_curl_command(curl_command)

            # Parse the response
            response_data = parse_curl_response(response_text)
            ...
            if retry_count < max_retries:
                ...
                try:
                    health_check_cmd = build_curl_command("GET", f"{BASE_URL}/health", timeout=5)
                    health_check_response, health_check_status = execute_curl_command(health_check_cmd)
                    if health_check_status != 200:
                        raise ValueError(f"Backend health check failed with status {health_check_status}")
...
"""
    ...
    try:
        # Build curl command for POST request with headers
 ...
 |                |                   |                  |
    ...
    while retry_count < max_retries:
        try:
            ...
            if status_code == 200:
                ...
                try:
                    ...
                    if isinstance(error_data, dict):
                        ...
                        error_message = str(error_data) if error_data else f"Status code: {status_code}"
                except Exception:
                    error_message = f"Status code: {status_code}"
            ...
            print(TermColors.colorize("⚠️ Request timed out. Retrying...", TermColors.YELLOW))

            retry_count += 1
            if retry_count >= max_retries:
                raise ValueError(f"Chart generation timed out after {max_retries} attempts")
            ...
            print(TermColors.colorize(f"⚠️ Error: {str(e)}. Retrying...", TermColors.YELLOW))

            retry_count += 1
            if retry_count >= max_retries:
                raise ValueError(f"Chart generation failed after {max_retries} attempts: {str(e)}")
...
def complete_questionnaire(questionnaire_id: str, session_id: str) -> Dict[str, Any]:
    """
    Complete a questionnaire and prepare for rectification.
...
"""
    ...
    while retry_count < max_retries:
        try:
            ...
            # If endpoint returned 404, the API might have a different structure than expected
            if response.status_code == 404:
                ...
                # Raise an exception to fail the test rather than trying a fallback
...
def rectify_birth_time(chart_id: str, questionnaire_id: str, session_id: str, answered_questions: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    """
...
"""
    ...
    try:
        ...
        if websocket_setup_result.get("status") == "success":
            ...
            # This case shouldn't happen due to error raising in setup_websocket_connection
            raise ConnectionError(f"WebSocket setup failed: {websocket_setup_result.get('message', 'Unknown error')}")

    except Exception as e:
        # Report the actual WebSocket error and fail the test
        error_message = f"Failed to establish WebSocket connection: {str(e)}"
        logger.error(error_message)
        print(TermColors.colorize(error_message, TermColors.RED))

        # Don't proceed with a failed WebSocket connection - this is a test failure
        raise RuntimeError(f"WebSocket connection required for real-time progress updates: {str(e)}")
...
"""
    ...
    try:
        ...
        # Build curl command for GET request with headers
        curl_command = build_curl_command(
            "GET",
            url,
            headers=headers,
        ...
        if status_code >= 200 and status_code < 300:
            # Parse the response
            chart_data = parse_curl_response(response_text)
            ...
            # Handle error response
            error_data = {"error": f"HTTP {status_code}", "response": response_text}
            elapsed_time = time.time() - start_time
            ...
            print(TermColors.colorize(f"Error retrieving rectified chart: HTTP {status_code}", TermColors.RED))
            raise ValueError(f"Failed to retrieve rectified chart: HTTP {status_code}")
    ...
    while retry_count < max_retries:
        try:
            ...
            try:
                ...
                response_data = {"text": response_text, "status_code": status_code}
            ...
            logger.error(f"Error during chart comparison: {str(e)}")
            print(TermColors.colorize(f"⚠️ Error: {str(e)}. Retrying...", TermColors.YELLOW))
...
def questionnaire_loop(chart_id: str, session_id: str) -> Dict[str, Any]:
    """
...
"""
    ...
    while current_question and question_number <= 10:  # Limit to 10 questions
        try:
            ...
            # Submit answer
            try:
                answer_result = answer_question(questionnaire_id, question_id, selected_answer, session_id)
...
def main(args: argparse.Namespace) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if args.test_type == "interactive":
            ...
            try:
                questionnaire_result = questionnaire_loop(chart_id, session_id)
                if questionnaire_result.get("status") == "success":
        ...
        try:
            ...
            # Pass the answered questions to the rectification function
            rectification_result = rectify_birth_time(chart_id, questionnaire_id, session_id, answered_questions)
        except Exception as e:
...
"""
    ...
    while retry_count < max_retries:
        try:
            ...
            if status_code in [200, 201, 202]:
                ...
                if download_url:
                    ...
                    try:
                        ...
                        # Build curl command for GET request with headers to download the file
                        download_headers = {"X-Session-ID": session_id}
                        download_curl_command = build_curl_command(
                            "GET",
                            download_url,
                        ...
                        # Execute curl command to download the file
                        download_response, download_status = execute_curl_command(download_curl_command)
                        ...
                        if download_status == 200:
                            ...
                            with open(download_path, 'wb') as f:
                                f.write(download_response.encode('latin1'))  # Use latin1 to preserve binary data
                            ...
                            print(TermColors.colorize(f"⚠️ Could not download file automatically. Status code: {download_status}", TermColors.YELLOW))
                    except Exception as download_error:
                        print(TermColors.colorize(f"⚠️ Error downloading file: {str(download_error)}", TermColors.YELLOW))
                        logger.warning(f"Error downloading file: {str(download_error)}")
            ...
            logger.error(f"Error during chart export: {str(e)}")
            print(TermColors.colorize(f"⚠️ Error: {str(e)}. Retrying...", TermColors.YELLOW))
...
"""
    ...
    try:
        ...
        if status_code >= 200 and status_code < 300:
            ...
            print(TermColors.colorize(f"Error retrieving chart: HTTP {status_code}", TermColors.RED))
            raise ValueError(f"Failed to retrieve chart: HTTP {status_code}")
...
"""
    ...
    try:
        ...
        # If WebSocket connection fails, fall back to a simulated connection
        # This ensures the test can still run even if WebSocket isn't available
        print(TermColors.colorize("Falling back to simulated WebSocket connection", TermColors.YELLOW))

        return {
            "status": "simulated",
            "connection_id": connection_id,
            "message": "WebSocket connection simulated due to connection error"
        }
...
"""
    ...
    if not hasattr(globals(), 'ws_connection') or ws_connection is None:
        ...
        logger.error(error_message)
        print(TermColors.colorize(error_message, TermColors.RED))
        # Don't fall back to simulation, report the real error
