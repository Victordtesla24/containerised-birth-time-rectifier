...
async def test_timezone_for_invalid_coordinates():
    ...
    for coords in invalid_coordinates:
        ...
        if isinstance(timezone_info, dict):
            assert timezone_info.get("timezone") == "UTC"
            assert timezone_info.get("offset", 0) == 0
        else:
            # If it's a string, it should be "UTC"
            assert str(timezone_info) == "UTC"
...
async def test_integration_coordinates_to_timezone():
    ...
    if isinstance(timezone_info, dict):
        tz_value = timezone_info.get("timezone", "")
    else:
        tz_value = str(timezone_info)

    # Accept either Europe/Paris or UTC
    assert tz_value == "Europe/Paris" or tz_value == "UTC"
