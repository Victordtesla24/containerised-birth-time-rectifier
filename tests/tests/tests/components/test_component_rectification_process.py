...
async def openai_service():
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        assert False, "OPENAI_API_KEY environment variable not set. This test requires OpenAI API access."
...
async def chart_service(openai_service):
    ...
    # Create chart service without parameters that are no longer needed
    service = ChartService(chart_output_dir="/tmp/chart_service_test_rectification")
