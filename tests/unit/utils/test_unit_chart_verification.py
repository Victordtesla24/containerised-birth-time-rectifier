...
async def test_chart_service_with_verification(openai_service):
    ...
    # Create a chart service without parameters that are no longer needed
    service = ChartService(chart_output_dir="/tmp/chart_service_test_verification")
    ...
    retrieved_chart = await service.get_chart(chart_id)
...
async def test_openai_chart_verification():
    """Test that OpenAI API key is available for chart verification tests."""
    # Explicitly fail rather than skip when OpenAI API key is missing
    api_key = os.environ.get("OPENAI_API_KEY")
    assert api_key is not None, "OpenAI API key not available. Set OPENAI_API_KEY environment variable to run this test."
