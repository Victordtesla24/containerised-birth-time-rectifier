...
try:
    ...
    # Don't skip - assert failure explicitly
    assert False, f"Failed to import ai_service.main module: {e}"
...
async def test_health_endpoint_exists():
    """Test that the health endpoint exists and returns a valid response."""
    try:
        # Import the FastAPI app
        import ai_service.main
        assert ai_service.main is not None, "Failed to import ai_service.main module"

        # Get the FastAPI application
        app = ai_service.main.app
        assert app is not None, "FastAPI app was not initialized"

        # Find health endpoint in routes
        ...
        assert has_health_endpoint, "Health endpoint not found in FastAPI routes"
