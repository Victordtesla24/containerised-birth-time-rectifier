...
async def create_chart_service():
    ...
    if hasattr(openai_service, 'http_client'):
        register_http_client(openai_service.http_client)
    ...
    try:
        ...
        chart_service = ChartService(chart_output_dir="/tmp/test_charts")
...
# Create a test implementation of DynamicQuestionnaireService
class TestQuestionnaireService(DynamicQuestionnaireService):
    """Extended DynamicQuestionnaireService with test-specific methods."""

    async def initialize_questionnaire(self, chart_id: str, session_id: str) -> dict:
        """
        Initialize a questionnaire for testing purposes.
        ...
        return {
            ...
            "question": {
                "id": "test_q1",
                "text": "Describe significant life events that occurred around ages that are multiples of 7 (7, 14, 21, 28, etc.)",
                "type": "text",
    ...
    async def get_next_question(
        ...
        """
        Get the next question or process an answer.

        Args:
            session_id: ID of the session
            chart_id: ID of the chart
            answer: Optional answer to the previous question
            question_id: Optional ID of the previous question

        Returns:
            Dictionary with the next question or completion status
        """
        if not question_id or question_id == "test_q1":
            # First/second question
            return {
                "question": {
                    "id": "test_q2",
                    "text": "What time of day were you born? (Morning, Afternoon, Evening, Night)",
                    "type": "multiple_choice",
                    "options": [
                        {"value": "morning", "text": "Morning (6am-12pm)"},
                        {"value": "afternoon", "text": "Afternoon (12pm-6pm)"},
                        {"value": "evening", "text": "Evening (6pm-10pm)"},
                        {"value": "night", "text": "Night (10pm-6am)"}
                    ],
                    "category": "birth_time"
                }
            }
        elif question_id == "test_q2":
            # Second/third question
            return {
                "question": {
                    "id": "test_q3",
                    "text": "Did you experience any major health events or physical traits that might correlate with your rising sign?",
                    "type": "text",
                    "category": "health"
                }
            }
        elif question_id == "test_q3":
            ...
            return {
                "question": {
                    ...
                    "type": "multiple_choice",
                    "options": [
                        {"value": "highly_introverted", "text": "Highly Introverted"},
                        {"value": "somewhat_introverted", "text": "Somewhat Introverted"},
                        {"value": "balanced", "text": "Balanced"},
                        {"value": "somewhat_extroverted", "text": "Somewhat Extroverted"},
                        {"value": "highly_extroverted", "text": "Highly Extroverted"}
                    ],
                    "category": "personality"
                }
            }
        else:
            # Indicate completion after answering all questions
            return {"complete": True}

    async def complete_questionnaire(self, session_id: str, chart_id: Optional[str] = None) -> dict:
        """
        Complete the questionnaire and return analysis results.

        Args:
            session_id: The session ID containing answers
        ...
        return {
            ...
            "birth_time_indicators": [
                ...
                {
                    "indicator": "personality_traits",
                    "value": "balanced",
                    "confidence": 65
                }
            ],
            "analysis_summary": "Based on the answers provided, birth time is likely in the early afternoon.",
            "confidence": 75,
            "recommendations": [
                "Consider the rectified birth time of 14:25:00"
            ]
        }
...
async def test_full_sequence_flow_real_api():
    ...
    try:
        ...
        # Check if timezone is a string or a dict with timezone field
        if isinstance(timezone_info, dict):
            assert "timezone" in timezone_info, "Timezone missing from result"
        ...
        questionnaire_service = TestQuestionnaireService()
        ...
        # First get the chart data and ensure it's not None
        export_chart_data = await chart_service.get_chart(rectified_chart_id)

        # Verify we have valid chart data
        if not export_chart_data:
            raise ValueError("Cannot export chart: Chart data not found")

        export_result = chart_service.export_chart(
