...
def run_all_tests():
    ...
    for test_file in test_files:
        ...
        # Run the test using pytest directly with subprocess to avoid asyncio conflicts
        import subprocess
        cmd = [sys.executable, "-m", "pytest", "-xvs", str(test_path)]
    ...
    if sequence_test.exists():
        cmd = [sys.executable, "-m", "pytest", "-xvs", str(sequence_test)]
        result = subprocess.run(cmd, capture_output=True, text=True)
        ...
        if result.returncode != 0:
            logger.error(f"Sequence flow test failed with exit code {result.returncode}")
            return result.returncode
